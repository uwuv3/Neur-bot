...
class Database {
  constructor(file) {
    ...
    if(this.file === 'database.json') {
      ...
      if(extension(this.file) !== 'json') throw Error("[err] the database file must end with .json")
      try { load(this.file); } catch { write(this.file, {}); }
    }
  }

  setBackup(filePath) {
    if (!filePath) throw Error("[err] a .json file must be mentioned for backups"); //err msg, tr/s4½
    if (extension(filePath) !== "json") throw Error("[err] the backup file must end with .json");
  ...
  loadBackup() {
    if (backup === undefined) throw Error("[err] can't find a backup || a file to load");
  ...
  remove(data) {
    ...
    let fileData = load(this.file)
    if(!fileData[data]) throw Error("[err] mentioned data isn't in directory or cannot be reached")
  ...
  add(data, value) {
    if (!data) throw Error("[err] no data to add");
	if (!value) throw Error("[err] no value to add");
 ...
 //made with ❤ by pythonic
  subtract(data, value) {
    if (!data) throw Error("[err] No data to subtract");
    if (!value) throw Error("[err] No value to subtract");
    if (typeof value !== "number") throw Error(`[err] The value to substract must be a number, received type: ${typeof value}`);
    let fileData = load(this.file)
 ...
 //db.map shall b€ inclU€d for v2.
  ...
  delete(array, index) {
    ...
    if (index === undefined) throw Error("[err] No index/value to delete from the array")
    let fileData = load(this.file)
    if (!fileData[array] && !Array.isArray(fileData[array])) throw Error("[err] The array to index/value delete dosen't exist or it's not array")
 ...
 //<html> forKey: "all-destroy" </html>
  deleteKey(object, key) {
    ...
    let fileData = load(this.file);
    if (!fileData[object]) throw Error("[err] The object to delete key dosen't exist in the database");
    if (typeof fileData[object] !== 'object') throw Error("[err] The provided object to key delete is not an object in the database");
...
//--------------------------------\\
  ...
  objectFetch(object, key) {
    ...
    if (!fileData[object]) throw Error("[err] The object to object fetch dosen't exist in the database");
    if (typeof fileData[object] !== 'object') throw Error("[err] The provided object to object fetch is not an object in the database");
  ...
  arrayFetch(array, number) {
    ...
    if (!number && number != 0) throw Error("[err] No index/number to array fetch")
    if (!fileData[array] && !Array.isArray(fileData[array])) throw Error("[err] The array to fetch dosen't exist or it's not array")
    if (typeof number !== "number" && value !== 0) throw Error(`[err] The number/index to array fetch must be a number, received type: ${typeof value}`);
  ...
  math(data, operator, value) {
    if (!data) throw Error("[err] No data to math")
    if (!operator) throw Error("[err] No operator to math")
    if (!value) throw Error("[err] No value to math")
    if (typeof value !== "number") throw Error(`[err] The value to math must be a number, received type: ${typeof value}`);
    let fileData = load(this.file)
    if (operator === "-") {
      ...
      throw Error("[err] unvaild operator to math, you can use only (-, +, *, /) (math) function")
    }
  }
}
